 ...
 * and state. NOTE: Clone of androidx.core.util.Preconditions
 */
public class Preconditions {
   ...
   */
  public static void checkState(final boolean expression, @Nullable String message) {
    if (!expression) {
  ...
  public static float checkArgumentInRange(
    ...
    if (Float.isNaN(value)) {
      ...
      throw new IllegalArgumentException(
          String.format(
              Locale.US, "%s is out of range of [%f, %f] (too low)", valueName, lower, upper));
    } else if (value > upper) {
      throw new IllegalArgumentException(
          String.format(
              Locale.US, "%s is out of range of [%f, %f] (too high)", valueName, lower, upper));
    }
  ...
  public static int checkArgumentInRange(int value, int lower, int upper, String valueName) {
    if (value < lower) {
      throw new IllegalArgumentException(
          String.format(
              Locale.US, "%s is out of range of [%d, %d] (too low)", valueName, lower, upper));
    } else if (value > upper) {
      throw new IllegalArgumentException(
          String.format(
              Locale.US, "%s is out of range of [%d, %d] (too high)", valueName, lower, upper));
    }
  ...
  /**
   * Method to check if the entered mobile number is a valid mobile number.
   *
   * @param mobileNumber entered mobile number
   * @return true or false depending on the mobile number
   */
  public static boolean isValidMobileNumber(String mobileNumber) {
    //if (mobileNumber.matches("^[0-9]{10}$")) {
    return !mobileNumber.startsWith("0")
        && !mobileNumber.startsWith("1")
        && !mobileNumber.startsWith("2")
        && !mobileNumber.startsWith("3")
        && !mobileNumber.startsWith("4")
